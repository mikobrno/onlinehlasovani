...
interface BuildingCSVImportModalProps {
  onClose: () => void;
}

export function BuildingCSVImportModal({ onClose }: BuildingCSVImportModalProps) {
  ...
  const handleImport = async () => {
    ...
    try {
      ...
      dataLines.forEach(line => {
        const [name, address, description, isActive] = line.split(',').map(cell => cell.trim());
        if (name && address) {
          addBuilding({
            name,
            address,
            description: description || undefined,
            is_active: isActive === 'Ano' || isActive === 'true' || isActive === '1'
          });
        }
      });
  ...
  const exampleCSV = `name,address,description,is_active
Bytový dům Náměstí míru,Náměstí míru 12; 120 00 Praha 2,Historický bytový dům,Ano
Rezidence Karlín,Thámova 15; 180 00 Praha 8,Moderní bytový komplex,Ano
Vila Vinohrady,Korunní 85; 120 00 Praha 2,Luxusní vila,Ne`;
    ...
    return (
      <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
        <div className="bg-white rounded-lg max-w-md w-full p-6 text-center">
          ...
          <p className="text-gray-600">Budovy byly úspěšně importovány.</p>
        </div>
      </div>
    );
  }
      ...
      <div className="bg-white rounded-lg max-w-4xl w-full max-h-[90vh] overflow-y-auto">
        <div className="flex items-center justify-between p-6 border-b border-gray-200">
          <div className="flex items-center space-x-3">
            ...
            <h2 className="text-xl font-semibold text-gray-900">Import budov z CSV</h2>
          </div>
          <button
            onClick={onClose}
            ...
            <div className="flex items-start space-x-3">
              ...
              <div>
                ...
                <p className="text-sm text-blue-700 mt-1">
                  CSV soubor musí obsahovat sloupce: name, address, description, is_active
                </p>
              </div>
            </div>
          </div>
